...
class CoverStoryHorizontalCell: UICollectionViewCell {
    ...
    let newSourceImageView : UIImageView = {
        ...
        imageView.image = Int.random(in: 0...1) == 0 ? UIImage(named: "newsSourceLogo") : UIImage(named: "infobaeLogo")
        imageView.clipsToBounds = true
        imageView.translatesAutoresizingMaskIntoConstraints = false
        return imageView
...
extension CoverStoryHorizontalCell: NewsCellProtocol {
    func configure(_ news: News) {
        headerImageView.image = UIImage(named: news.image)
